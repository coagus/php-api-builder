...
8. **Functional CI tests are the release's definition of done.** A release is not "shipped" until GitHub Actions is fully green. The loop is: completed development → local tests + code validations (container) → if green, release → CI runs the `test-api` functional suite → if green, done → if red, retry cycle. Max **3 iterations** (`alpha.N`, `alpha.N+1`, `alpha.N+2`). On the 3rd red, **STOP and notify the user explicitly** (surface in the final message and, if available, a `PushNotification`). Never auto-retry beyond 3.
